...
-- type instance Sort' (a :: [Type]) = SortType a

-- type family SortType (xs :: [Type]) :: [Type] where 
--     SortType ('[]) = '[]
--     SortType (x ': '[]) = '[x]
--     SortType (x : xs) = SortType (Fst (Main.Split x xs)) ++ (x ': SortType (Snd (Main.Split x xs)))

-- type family Split (x :: Type) (xs :: [Type]) :: ([Type], [Type]) where
--     Main.Split x ('[]) = '( '[], '[])
--     Main.Split x (y : ys) = If (Less (CmpType x y)) 
--         '(y : (Fst (Main.Split x ys)), (Snd (Main.Split x ys))) '((Fst (Main.Split x ys)), y : (Snd (Main.Split x ys)))

-- type family Fst (pair :: ([k], [k])) :: [k] where
--     Fst '(xs, ys) = xs

-- type family Snd (pair :: ([k], [k])) :: [Type] where
--     Snd '(xs, ys) = ys
...
-- type Sorted s = Sort s

-- sort :: List s -> List (Sorted s)
-- sort Empty = Empty
-- sort (Const x xs) = 

-- f :: Proxy (Sort' [Bool, Int]) -> Proxy (Sort' [Int, Bool])
-- f = (Cons True (Cons 1 Nil))

-- data List (n :: [k]) :: * where
--     Nil :: List '[]
--     Cons :: e -> List s -> List (e ': s)
...
-- --     DeleteFromList elem '[] = '[]
-- --     DeleteFromList elem (x ': xs) = If (CmpType elem x == EQ)
-- --                                        xs
